...
xg_model_caret = train(npp~., data = df_wy, method = "xgbTree", trControl = train_control)
```
...
```{r}
c_imp = varImp(xg_model_caret, scale = FALSE)
list_imp = c_imp$importance
list_imp = list_imp %>% mutate(variable = row.names(list_imp)) %>% 
  filter(list_imp$Overall != 0)

ggplot(data = list_imp, aes(x=Overall, y=variable)) + geom_col()
```
