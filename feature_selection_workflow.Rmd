...
**Added plots of preliminary importance and VIF showing selected and removed variables and summary table containing all that info. Maybe we could include a pairs plot too to show correlation, or include max correlation plot / summary.**
...
Testing the rfUtilities

```{r}
multi.collinear(numericpredictors.df_wy0, p=.05)
```
...
```{r}
rf.classBalance(ydata=df_wy0_reduced[,"npp"],
                xdata=df_wy0_reduced[,2:ncol(df_wy0_reduced)])
```
...
```{r}
rf.regression.fit(rf_wy0)
```

```{r}
rf.significance(rf_wy0,
                allpredictors.df_wy0_reduced,
                p=.1,
                nperm=10)	
```

```{r}
# worth showing other importance measures and explaining why we don't use them?
varImpPlot(rf_wy0)
```
...
#not sure we want to be forcing people to install miniconda
```
