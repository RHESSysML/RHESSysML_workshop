...
```{r}
write.csv(df_wy, file = here("aggregated_datasets", "df_wy.csv"), row.names = FALSE)
write.csv(df_wy0, file = here("aggregated_datasets", "df_wy0.csv"), row.names = FALSE)
write.csv(df_wy2, file = here("aggregated_datasets", "df_wy2.csv"), row.names = FALSE)
```
...
# Set preference order based on variable importance
preference.order0 <- varImp(imp0) %>% arrange(-Overall) %>% rownames()

# Second climate scenario

# Find preliminary importance using random forest
imp2 <- randomForest(formula=npp~.,
                     data=df_wy2,
                     replace=TRUE,
                     importance=TRUE,
                     keep.forest=TRUE,
...
# Set preference order based on variable importance
preference.order2 <- varImp(imp2) %>% arrange(-Overall) %>% rownames()
...
wy0_vif <- remove_vif(predictors.df=allpredictors.df_wy0, vif.threshold=5, preference.order=preference.order0)$selected.variables
wy0_cor <- remove_cor(predictors.df=allpredictors.df_wy0, cor.threshold=0.75, preference.order=preference.order0)$selected.variables
wy0_select_variables <- unique(append(wy0_vif, wy0_cor))

wy2_vif <- remove_vif(predictors.df=allpredictors.df_wy2, vif.threshold=5, preference.order=preference.order2)$selected.variables
wy2_cor <- remove_cor(predictors.df=allpredictors.df_wy2, cor.threshold=0.75, preference.order=preference.order2)$selected.variables
...
ggsave(importance_plots, filename = "importance_plots.png", path = here("www"))
```
...
pred1_clim0 <- get_important_predictors(df_imp_wy0, 1)
pred2_clim0 <- get_important_predictors(df_imp_wy0, 2)
pred1_clim2 <- get_important_predictors(df_imp_wy2, 1)
pred2_clim2 <- get_important_predictors(df_imp_wy2, 2)

create_binned_df <- function(df_wy) {
  df_pred_binned <- df_wy %>% 
  select(all_of(pred1_clim0), all_of(pred2_clim0), all_of(response_var)) %>%
...
create_binned_plot <- function(df_binned) {
  ...
  ggplot(df_binned, aes(x = pred1, y = .data[[response_var]])) +
    geom_point() +
    geom_smooth(method = "lm") +
    facet_wrap("bin") +
    labs(x = str_to_title(pred1_clim0), y = response_var,
...
explain0 <- explain(object=rf_wy0,
