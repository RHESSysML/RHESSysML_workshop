...
Since temporal aggregation of the data set is critical to the results, we first create a function that allows us to dynamically select whether monthly or seasonal average temperatures are included as predictor variables.
...
Next, the necessary modifications to the data set are made. For this example, we are converting factor variables, aggregating by water year, changing `slope` and `aspect` from radians to degrees, and adding two derived variables: `peak_swe` and `swe_precip_ratio`. 
...
wy0_vif <- remove_vif(df_wy0_num_preds, vif.threshold = 5, pref_order0)$selected.variables
wy0_cor <- remove_cor(df_wy0_num_preds, cor.threshold = 0.75, pref_order0)$selected.variables
wy0_select_variables <- unique(append(wy0_vif, wy0_cor))

wy2_vif <- remove_vif(df_wy2_num_preds, vif.threshold = 5, pref_order2)$selected.variables
wy2_cor <- remove_cor(df_wy2_num_preds, cor.threshold = 0.75, pref_order2)$selected.variables
...
Next, we run the function for both climate scenario data sets and visualize the results.
