...
num_preds.df_wy0 <- df_wy0[,(num_response+1):ncol(df_wy0)] %>% 
  select(where(is.numeric))

factor_preds.df_wy0 <- df_wy0 %>% 
  select(!response_var & where(is.factor)) %>% 
  colnames()

# Save data frames of predictor variables for second climate scenario
num_preds.df_wy2 <- df_wy2 %>% 
  select(!response_var & where(is.numeric))
...
preference.order0 <- varImp(imp0, scale=FALSE) %>% 
  arrange(-Overall) %>% 
  rownames()
...
# Set preference order based on variable importance
preference.order2 <- varImp(imp2, scale=FALSE) %>% 
  arrange(-Overall) %>% 
  rownames()
```
...
df_wy0_reduced <- df_wy0 %>% 
  select(c(all_of(response_var), all_of(factor_preds.df_wy0), all_of(wy0_select_variables)))

df_wy2_reduced <- df_wy2 %>% 
  select(c(all_of(response_var), all_of(factor_preds.df_wy2), all_of(wy2_select_variables)))
```
